...
def pipeline_1_facet() -> list[dict]:
    return [
...
def pipeline_2_rolling_avg() -> list[dict]:
    return [
        {"$match": {"event_type": "pipeline", "duration_seconds": {"$gt": 0}}},
        {"$sort": {"timestamp": 1}},
        {
            "$group": {
        ...
        {"$sort": {"_id": 1}},
        {
            "$setWindowFields": {
                ...
                "sortBy": {"_id": 1},
                "output": {
                    "rolling_avg_7d": {"$avg": "$avg_duration", "window": {"documents": [-6, 0]}},
                    "cumulative_count": {"$sum": "$count", "window": {"documents": ["unbounded", "current"]}},
                },
            },
        },
        {"$project": {"date": "$_id", "avg_duration": 1, "rolling_avg_7d": 1, "count": 1, "cumulative_count": 1, "_id": 0}},
    ]


def pipeline_3_percentiles() -> list[dict]:
    return [
        {"$match": {"event_type": {"$in": ["build", "pipeline"]}, "duration_seconds": {"$gt": 0}}},
        {"$group": {"_id": "$team", "durations": {"$push": "$duration_seconds"}}},
        {"$project": {"team": "$_id", "durations": 1, "n": {"$size": "$durations"}}},
        {"$project": {"team": 1, "durations": {"$sortArray": {"input": "$durations", "sortBy": 1}}, "n": 1}},
        {
            "$project": {
                "team": 1,
                "n": 1,
...
def pipeline_4_bucket_histogram() -> list[dict]:
    return [
        {"$match": {"event_type": {"$in": ["build", "pipeline"]}, "duration_seconds": {"$gte": 0}}},
        {
            "$bucket": {
...
def pipeline_5_author_efficiency() -> list[dict]:
    return [
        ...
        },
        {
            "$addFields": {
                "success_rate": {
                    "$cond": [{"$gt": ["$total_events", 0]}, {"$divide": ["$success_count", "$total_events"]}, 0],
                },
                "sla_rate": {"$cond": [{"$gt": ["$total_events", 0]}, {"$divide": ["$sla_met_count", "$total_events"]}, 0]},
            },
        },
        {
            "$addFields": {
...
def pipeline_6_failure_correlation() -> list[dict]:
    return [
        {"$match": {"status": "failed", "failure_reason": {"$ne": ""}}},
        {
            "$group": {
        ...
        },
        {
            "$project": {
                ...
                "top_reasons": {
                    "$slice": [
                        {"$sortArray": {"input": "$reasons", "sortBy": {"count": -1}}},
                        5,
                    ]
                },
            },
